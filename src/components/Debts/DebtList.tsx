...
export const DebtList: React.FC<DebtListProps> = ({
  ...
  const getTypeInfo = (type: string) => {
    switch (type) {
      case 'lend':
        return { label: 'Cho mượn', icon: DollarSign, color: 'text-green-600' };
      case 'borrow':
        return { label: 'Mượn', icon: DollarSign, color: 'text-red-600' };
      case 'group_payment':
        return { label: 'Trả cho nhóm', icon: Users, color: 'text-blue-600' };
      case 'group_receive':
        return { label: 'Nhận từ nhóm', icon: User, color: 'text-purple-600' };
      default:
        return { label: 'Không xác định', icon: DollarSign, color: 'text-gray-600' };
    }
  };
  ...
  if (debts.length === 0) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-500">Chưa có khoản vay nợ nào được ghi nhận</p>
      </div>
    );
  }
            ...
            >
              ...
              <div className="border-t border-gray-200 pt-4">
                ...
                {isGroupType && debt.splits && debt.splits.length > 0 && (
                  <div className="mt-4">
                    <h4 className="text-sm font-medium text-gray-700 mb-2">Chi tiết chia tiền:</h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                      {debt.splits.map((split, index) => (
                        <div
                          key={index}
                          className={`flex items-center justify-between p-2 rounded border ${
                            split.isPaid ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-200'
                          }`}
                        >
                          <div className="flex items-center space-x-2">
                            {split.isPaid ? (
